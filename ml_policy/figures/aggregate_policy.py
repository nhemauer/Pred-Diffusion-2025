...
def combine_policy_files(policy_files_dir, output_dir):
    """
    Combine per-bill replication files from policy_files/ into one results file
    per model type. Each input file is named like:
      {author}_policy_results_{model}_{index}.csv
    and contains a single bill's result.

    Returns a dict of {model_key: DataFrame} for all models processed.
    """
    csv_files = glob.glob(os.path.join(policy_files_dir, '*.csv'))

    if not csv_files:
        return {}

    # Group files by (author, model) key, e.g. "mallinson_policy_results_rf"
    model_groups = {}
    pattern = re.compile(r'^(.+_policy_results_[a-zA-Z]+)_\d+$')
...
def load_existing_results(folder):
    """
    Load any *_policy_results_*.csv files already present in folder
    (not inside policy_files/) that were not generated from policy_files.
    """
    existing = {}
    for csv_file in folder.glob('*_policy_results_*.csv'):
        key = csv_file.stem
...
figures_dir = Path("ml_policy/figures")
...
if not figures_dir.exists():
    ...
    for folder in sorted(figures_dir.iterdir()):
        ...
        print(f"\n=== {folder.name} ===")
        policy_files_dir = folder / 'policy_files'

        # Start with any results files already sitting at the folder level
        all_model_dfs = load_existing_results(folder)

        # If there is a policy_files sub-directory, combine those files
        if policy_files_dir.is_dir():
            generated = combine_policy_files(policy_files_dir, folder)
        ...
        # "boehmke_policy_results_logit" -> "boehmke_policy_results"
    ...
    summary_path = figures_dir / 'policy_averages.csv'
