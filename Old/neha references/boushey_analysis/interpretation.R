...
load('./output_data/boushey_subset_oos_results_combined.RData')
...
library(xtable)
#library(devtools)
#install_github("desmarais-lab/dnehm")
library(neha)
...
neha_formula <- as.formula(paste("dvadopt ~",paste(c(covariates,edge_effs),collapse="+")))

# Table 2, column 1 (206) 
peha.boushey <- robcov(lrm(dvadopt ~ policycongruent + gub_election + elect2 + fedcrime + leg_dem_per_2pty + dem_governor + insession + propneighpol + citidist + squire_prof86 + citi6008 + crimespendpc + crimespendpcsq + violentthousand + pctwhite + stateincpercap + logpop + counter + counter2 + counter3, x = TRUE, y = TRUE, data = boushey_nona), cluster = boushey_nona$styear)
...
neha.boushey <- robcov(lrm(neha_formula, x = TRUE, y = TRUE, data = boushey_nona), cluster = boushey_nona$styear)

neha.bic <- BIC(glm(neha_formula, family=binomial,data=boushey_nona))

peha.bic <- BIC(glm(dvadopt ~ policycongruent + gub_election + elect2 + fedcrime + leg_dem_per_2pty + dem_governor + insession + propneighpol + citidist + squire_prof86 + citi6008 + crimespendpc + crimespendpcsq + violentthousand + pctwhite + stateincpercap + logpop + counter + counter2 + counter3, family=binomial,data=boushey_nona))

reg_table <- cbind(c(coef(peha.boushey),NA), c(sqrt(diag(peha.boushey$var)),NA) , c(coef(neha.boushey)), c(sqrt(diag(neha.boushey$var))))
reg_table <- rbind(reg_table,c(NA,NA,NA,neha_boushey[[1]]))
reg_table <- rbind(reg_table,c(nrow(boushey_nona),length(upol),NA,NA))
...
rownames(reg_table)[1:(nrow(reg_table)-4)] <- c("Intercept","Policy Congruence","Elect1","Elect2","National Crime Salience","Democratic Party Strength","Democratic Governor","Legislative Session","Neighbors","Ideological Distance","Legislative Professionalism","Political Ideology","Crime Control Spending","Crime Control Spending Squared","Violent Crime Rate","Pct. White","Per Capita Income","Log Population", "Time","Time Squared","Time Cubed")

write.csv(reg_table,"./results_summaries/regression_table.csv") 
xtable(reg_table,dig=5)

mean_lp <- mean(cbind(1,neha.boushey$x)%*%cbind(coef(neha.boushey)))
times <- 1:50
...
library(ggplot2)
df <- data.frame(years = times,diffusion_effect=plogis(mean_lp + coef(neha.boushey)[length(coef(neha.boushey))]*alpha_fun(times,exp(neha_boushey[[1]]))))
...
ggsave("./results_summaries/boushey_decay.pdf", width = 4, height = 3)
...
set.seed(9202011)
pdf("./results_summaries/boushey_edge_plot.pdf",height=4,width=4,pointsize=8)
...
for(s in 1:50){
	...
	load(paste("./output_data/neha_boushey_sim_subset",s,".RData",sep=""))
	sim_edges[[s]] <- edges_subset
	precision <- c(precision, mean(is.element(edges_subset,edge_effs)))
	neha_boushey_sim[[3]]$edge_sum <- apply(neha_boushey_sim[[3]][,edges_subset],1,sum)
	...
	load(paste("./output_data/neha_boushey_sim_subset_null",s,".RData",sep=""))
	sim_edges_zero[[s]] <- edges_subset
	n_null <- c(n_null,length(edges_subset))
	
	if(length(edges_subset) > 0){
		neha_boushey_sim[[3]]$edge_sum <- apply(cbind(neha_boushey_sim[[3]][,edges_subset]),1,sum)
		est_neha_sim <- glm(neha_formula,family=binomial,data=neha_boushey_sim[[3]])
...
p+ geom_vline(aes(xintercept=coef(neha.boushey)["edge_sum"]),
            ...
            color="red", size=1)
ggsave("./results_summaries/boushey_gamma_sim.pdf", width = 4, height = 3)


write.csv(n_null,"./results_summaries/n_null.csv")      
