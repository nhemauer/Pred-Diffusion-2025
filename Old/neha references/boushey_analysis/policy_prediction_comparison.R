...
library(xtable)
neha_top_5 <- data.frame(policy = policy_name[order(peha_auc/neha_auc)[1:5]], minyr = minyr[order(peha_auc/neha_auc)[1:5]], ratio = (peha_auc/neha_auc)[order(peha_auc/neha_auc)[1:5]]  )

neha_bot_5 <- data.frame(policy = policy_name[order(-peha_auc/neha_auc)[1:5]],minyr = minyr[order(-peha_auc/neha_auc)[1:5]], ratio = (peha_auc/neha_auc)[order(-peha_auc/neha_auc)[1:5]]       )

xtable(rbind(neha_top_5,neha_bot_5))

write.csv(rbind(neha_top_5,neha_bot_5),file='./results_summaries/top5bot5.csv')
