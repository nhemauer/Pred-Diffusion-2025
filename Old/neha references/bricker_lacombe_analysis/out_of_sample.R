...
system.time(neha_bl_train <- neha(data_train,node="state",time="year",event="adoption",cascade="policyno",covariates=covariates,ncore=20))
...
save(list=c("neha_bl_train","data_train"),file=paste("./output_data/bl_oos_subset_results",g,".RData",sep=""))
     ...
     neha_bl_train[[3]]$edge_sum <- rep(0,nrow(neha_bl_train[[3]]))
     test_data$edge_sum <- rep(0,nrow(test_data))

     for(i in 1:length(edges_inferred)){
       neha_bl_train[[3]]$edge_sum <- neha_bl_train[[3]]$edge_sum + neha_bl_train[[3]][,edges_inferred[i]]
