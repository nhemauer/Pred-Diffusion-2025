...
# Data
lacombe_boehmke_2021 = pd.read_csv(
    r"figures/lacombe_boehmke2021/lacombe_stata_results.csv", 
    nrows = 78,
    dtype = str 
)

# Clean the data by removing the =" and " from each cell (Stata...)
for col in lacombe_boehmke_2021.columns:
    lacombe_boehmke_2021[col] = lacombe_boehmke_2021[col].str.replace('="', '').str.replace('"', '')

# Grab needed data
selected_rows = pd.concat([
    lacombe_boehmke_2021.iloc[3:19], 
    lacombe_boehmke_2021.iloc[[74]]   
])
...
plt.tight_layout()
plt.savefig('figures/lacombe_boehmke2021/lacombe_coef_comparison.png', dpi = 300, bbox_inches = 'tight')
plt.show()