...
ggsave("./results_summaries/karch_decay.pdf", width = 4, height = 3)
...
reg_table <- rbind(reg_table,c(NA,NA,NA,neha_karch[[1]]))
...
set.seed(9202011)
pdf("./results_summaries/karch_edge_plot.pdf",height=4,width=4,pointsize=8)
...
for(s in 1:50){
  ...
  load(paste("./output_data/neha_karch_sim_subset",s,".RData",sep=""))
  ...
  load(paste("./output_data/neha_karch_sim_subset_zero",s,".RData",sep=""))
...
p+ geom_vline(aes(xintercept=coef(neha.karch)["edge_sum"]),
              color="blue", linetype="dashed", size=1) + geom_vline(aes(xintercept=mean(gamma)),
                                                                    color="red", size=1)
ggsave("./results_summaries/karch_gamma_sim.pdf", width = 4, height = 3)
