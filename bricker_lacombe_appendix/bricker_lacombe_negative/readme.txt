...
There is one post-analysis script that should be run following scripts 2--4. Script 1 is run by Script 3, so needn't be run on its own. The following script was not run on HPC, and so there is no pbs script.
