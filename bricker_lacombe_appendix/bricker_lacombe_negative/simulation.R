...
neha_formula <- as.formula(paste("adoption~",paste(c(covariates,"edge_sum_pos","edge_sum_neg"),collapse="+"),sep=""))

edge_effs  <- neha_bl[[2]]

covariates_neha <- c(covariates,edge_effs)
neha.s <- glm(neha_bl[[3]][,"adoption"] ~ as.matrix(neha_bl[[3]][, covariates_neha]),family="binomial")

edge_sign <- sign(coef(neha.s)[-(1:(length(covariates)+1))])

edge_effs_pos <- edge_effs[which(edge_sign>0)]
edge_effs_neg <- edge_effs[which(edge_sign < 0)]

neha_bl[[3]]$edge_sum_pos <- apply(neha_bl[[3]][,edge_effs_pos],1,sum)
neha_bl[[3]]$edge_sum_neg <- apply(neha_bl[[3]][,edge_effs_neg],1,sum)
...
gamma <- numeric(length(edge_sign))
gamma[which(edge_sign > 0 )] <- rep(neha_coef[length(covariates)+2],sum(edge_sign > 0) )
gamma[which(edge_sign < 0 )] <- rep(neha_coef[length(covariates)+3],sum(edge_sign < 0) )
gamma <- cbind(gamma)
...
#    times <- min(bl_nona_c$year):(max(bl_nona_c$year)+length(times))
    times <- (median(bl_nona_c$year)-10):(median(bl_nona_c$year)+10)	
  ...
  neha_bl_sim <- neha(sim_bl,node="state",time="sim_time",event="event",cascade="policyno",covariates=covariates,ncore=18,negative=T)
  ...
  save(list=c("edges_subset","neha_bl_sim"),file=paste("./output_data/neha_bl_sim_subset",s,".RData",sep=""))
