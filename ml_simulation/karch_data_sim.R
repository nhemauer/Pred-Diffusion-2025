...
# Data
karch_2016_full = read_dta("data/karch2016.dta")
...
karch_2016 <- karch_2016_full %>%
  select(state, year, compnum, adopt, all_of(covariates)) %>%
  na.omit()
...
logistic <- glm(formula, data = karch_2016, family = binomial(link = "logit"))
...
for (bill in unique(karch_2016$compnum)){
  # Filter data per bill
  policy_data <- karch_2016 %>% filter(compnum == bill)
...
write.csv(sim_results, "ml_simulation/figures/karch2016/karch_sim_data.csv", row.names = FALSE)