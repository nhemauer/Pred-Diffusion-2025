...
# Create lagged variables (exclude counter variables)
covariates_to_lag = [cov for cov in covariates if cov not in ['counter', 'counter2', 'counter3']]

for cov in covariates_to_lag:
    boushey_2016[f'{cov}_lag1'] = boushey_2016.groupby('state')[cov].shift(1)

for cov in covariates_to_lag:
    boushey_2016[f'{cov}_lag5'] = boushey_2016.groupby('state')[cov].shift(4)

# Drop rows with NaN values (from lagging)
boushey_2016 = boushey_2016.dropna()
...
with open("figures/boushey2016/t1_forecast_results_lag1.txt", "w") as f:
...
plt.tight_layout()
plt.savefig('figures/boushey2016/t1_forecast_timeseries_lag1.png', dpi = 300, bbox_inches = 'tight')
plt.show()
...
time_series_results.to_csv('figures/boushey2016/t1_forecast_timeseries_lag1.csv', index = False)