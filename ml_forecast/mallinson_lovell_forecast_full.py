...
with open("figures/mallinson_lovell2022/t1_forecast_results.txt", "w") as f:
    for model in ['original', 'logit', 'rf', 'xgb']:
        f.write(f"\n{model.upper()} Results:\n")
...
plt.tight_layout()
plt.savefig('figures/mallinson_lovell2022/t1_forecast_timeseries.png', dpi = 300, bbox_inches = 'tight')
plt.show()
...
time_series_results.to_csv('figures/mallinson_lovell2022/t1_forecast_timeseries.csv', index = False)
...
# Save aggregated results
with open("figures/mallinson_lovell2022/t5_forecast_results.txt", "w") as f:
    for model in ['original', 'logit', 'rf', 'xgb']:
        f.write(f"\n{model.upper()} Results:\n")
        f.write(f"Average AP Score (per year): {np.mean(results[model]['ap_score']):.4f} (±{np.std(results[model]['ap_score']):.4f})\n")
        f.write(f"Overall AP Score (all predictions): {overall_ap_scores[model]:.4f}\n")
...
plt.tight_layout()
plt.savefig('figures/mallinson_lovell2022/t5_forecast_timeseries.png', dpi = 300, bbox_inches = 'tight')
plt.show()
...
time_series_results.to_csv('figures/mallinson_lovell2022/t5_forecast_timeseries.csv', index = False)
