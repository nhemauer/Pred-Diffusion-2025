...
for train_end_year in range(mid_year, max_year):
    ...
    # Prepare features
    X_train = train_data.drop(columns = ['adoption', 'policyno', 'state'])
    X_val = val_data.drop(columns = ['adoption', 'policyno', 'state'])
    X_test = test_data.drop(columns = ['adoption', 'policyno', 'state'])
    
    # Create dummy variables for ALL possible years in the dataset
    all_years = sorted(lacombe_boehmke2021['year'].unique())
...
with open("figures/lacombe_boehmke2021/t1_forecast_results.txt", "w") as f:
...
plt.tight_layout()
plt.savefig('figures/lacombe_boehmke2021/t1_forecast_timeseries.png', dpi = 300, bbox_inches = 'tight')
plt.show()
...
time_series_results.to_csv('figures/lacombe_boehmke2021/t1_forecast_timeseries.csv', index = False)
...
# Save aggregated results
with open("figures/lacombe_boehmke2021/t5_forecast_results.txt", "w") as f:
    for model in ['original', 'logit', 'rf', 'xgb']:
        f.write(f"\n{model.upper()} Results:\n")
        f.write(f"Average AP Score: {np.mean(results[model]['ap_score']):.4f} (±{np.std(results[model]['ap_score']):.4f})\n")
...
plt.tight_layout()
plt.savefig('figures/lacombe_boehmke2021/t5_forecast_timeseries.png', dpi = 300, bbox_inches = 'tight')
plt.show()
...
time_series_results.to_csv('figures/lacombe_boehmke2021/t5_forecast_timeseries.csv', index = False)
...
# Save aggregated results
with open("figures/lacombe_boehmke2021/t10_forecast_results.txt", "w") as f:
    for model in ['original', 'logit', 'rf', 'xgb']:
        f.write(f"\n{model.upper()} Results:\n")
        f.write(f"Average AP Score: {np.mean(results[model]['ap_score']):.4f} (±{np.std(results[model]['ap_score']):.4f})\n")
...
plt.tight_layout()
plt.savefig('figures/lacombe_boehmke2021/t10_forecast_timeseries.png', dpi = 300, bbox_inches = 'tight')
plt.show()
...
time_series_results.to_csv('figures/lacombe_boehmke2021/t10_forecast_timeseries.csv', index = False)