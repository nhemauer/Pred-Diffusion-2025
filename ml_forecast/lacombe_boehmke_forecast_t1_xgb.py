...
with open("figures/lacombe_boehmke2021/t1_forecast_results_xgb.txt", "w") as f:
    for model in ['xgb']:
        f.write(f"\n{model.upper()} Results:\n")
...
years = list(range(mid_year + 2, mid_year + 2 + len(results['xgb']['ap_score'])))
...
plt.tight_layout()
plt.savefig('figures/lacombe_boehmke2021/t1_forecast_timeseries_xgb.png', dpi = 300, bbox_inches = 'tight')
plt.show()
...
time_series_results.to_csv('figures/lacombe_boehmke2021/t1_forecast_timeseries_xgb.csv', index = False)