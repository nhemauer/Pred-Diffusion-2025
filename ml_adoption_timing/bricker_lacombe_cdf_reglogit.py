...
# Define X and y
X_full = bricker_lacombe_2021[['year'] + covariates].copy()
X_full = pd.get_dummies(X_full, columns = ['year'], drop_first = True)
y_full = bricker_lacombe_2021['adoption']

os.chdir("ml_adoption_timing")

### Cross-validation on policies

unique_policies = bricker_lacombe_2021['policy'].unique()
...
for fold_idx, (train_idx, test_idx) in enumerate(kfold.split(unique_policies)):
    ...
    train_mask = bricker_lacombe_2021['policy'].isin(train_policies)
    test_mask = bricker_lacombe_2021['policy'].isin(test_policies)
    ...
    for policy in test_policies:
        ...
        policy_data = bricker_lacombe_2021[bricker_lacombe_2021['policy'] == policy]
...
results_df.to_csv("figures/bricker_lacombe2021/bricker_lacombe_cdf_results_reglogit.csv", index = False)
...
with open('figures/bricker_lacombe2021/bricker_lacombe_maef1_reglogit.txt', 'w') as f:
