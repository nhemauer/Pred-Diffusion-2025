...
mallinson_2019 = mallinson_2019_full[["adopt", "state", "year", "policy"] + covariates].dropna()

# Define X and y
X_full = mallinson_2019.drop(columns = ['adopt', 'state', 'year', 'policy']).copy()
y_full = mallinson_2019['adopt']
...
for fold_idx, (train_idx, test_idx) in enumerate(kfold.split(unique_policies)):
    ...
    train_mask = mallinson_2019['policy'].isin(train_policies)
    test_mask = mallinson_2019['policy'].isin(test_policies)
    ...
    for policy in test_policies:
        ...
        policy_data = mallinson_2019[mallinson_2019['policy'] == policy]
        ...
        for state in policy_data['state'].unique():
            ...
            for extension in range(max_extensions):
                # Prepare features
                X_state_policy = extended_data[covariates].copy()
...
results_df.to_csv("figures/mallinson2019/mallinson_cdf_results_reglogit.csv", index = False)
...
with open('figures/mallinson2019/mallinson_maef1_reglogit.txt', 'w') as f:
