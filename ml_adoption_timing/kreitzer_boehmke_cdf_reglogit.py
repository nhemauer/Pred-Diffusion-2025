...
# Define X and y
X_full = kreitzer_boehmke_2016[covariates + ['policy_num']].copy()
X_full = pd.get_dummies(X_full, columns = ['policy_num'], drop_first = True)
y_full = kreitzer_boehmke_2016['adopt_policy']

os.chdir("ml_adoption_timing")

### Cross-validation on policies

unique_policies = kreitzer_boehmke_2016['policy_num'].unique()
...
for fold_idx, (train_idx, test_idx) in enumerate(kfold.split(unique_policies)):
    ...
    train_mask = kreitzer_boehmke_2016['policy_num'].isin(train_policies)
    test_mask = kreitzer_boehmke_2016['policy_num'].isin(test_policies)
    ...
    for policy in test_policies:
        ...
        policy_data = kreitzer_boehmke_2016[kreitzer_boehmke_2016['policy_num'] == policy]
        ...
        for state in policy_data['state'].unique():
            ...
            for extension in range(max_extensions):
                # Prepare features - add policy_num back as column
                X_state_policy = extended_data[covariates + ['policy_num']].copy()
                X_state_policy = pd.get_dummies(X_state_policy, columns = ['policy_num'], drop_first = True)
                ...
                hazards = reglogit_model.predict_proba(X_state_policy_scaled)[:, 1]
                ...
                if extension < max_extensions - 1:
                    ...
                    # Impute covariates for the new year (don't impute policy_num - it stays constant)
                    ...
                    last_row['adopt_policy'] = 0  # Not yet adopted
                    # policy_num stays the same (already in last_row)
                    
                    # Add to extended data
                    extended_data = pd.concat([extended_data, last_row], ignore_index = True)
            
            # Find actual adoption time
            adoption_years = state_policy_data[state_policy_data['adopt_policy'] == 1]['year'].values
...
results_df.to_csv("figures/kreitzer_boehmke2016/kreitzer_boehmke_cdf_results_reglogit.csv", index = False)
...
with open('figures/kreitzer_boehmke2016/kreitzer_boehmke_maef1_reglogit.txt', 'w') as f:
