...
boehmke_2017 = boehmke_2017_full[["state", "year", "policy", "adopt"] + covariates].dropna()

# Define X and y
X_full = boehmke_2017.drop(columns = ['adopt', 'year', 'policy']).copy()
X_full = pd.get_dummies(X_full, columns = ['state'], drop_first = True)
y_full = boehmke_2017['adopt']

os.chdir("ml_adoption_timing")

### Cross-validation on policies

unique_policies = boehmke_2017['policy'].unique()
...
for fold_idx, (train_idx, test_idx) in enumerate(kfold.split(unique_policies)):
    ...
    train_mask = boehmke_2017['policy'].isin(train_policies)
    test_mask = boehmke_2017['policy'].isin(test_policies)
    ...
    for policy in test_policies:
        ...
        policy_data = boehmke_2017[boehmke_2017['policy'] == policy]
        ...
        for state in policy_data['state'].unique():
            ...
            for extension in range(max_extensions):
                # Prepare features
                X_state_policy = extended_data[['state'] + covariates].copy()
                X_state_policy = pd.get_dummies(X_state_policy, columns = ['state'], drop_first = True)
...
results_df.to_csv("figures/boehmke2017/boehmke_cdf_results_reglogit.csv", index = False)
...
with open('figures/boehmke2017/boehmke_maef1_reglogit.txt', 'w') as f:
