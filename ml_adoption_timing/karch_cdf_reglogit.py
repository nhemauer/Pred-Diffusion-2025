...
karch_2016 = karch_2016_full[["adopt", "compnum", "state", "year"] + covariates].dropna()

# Define X and y
X_full = karch_2016[covariates].copy()
y_full = karch_2016['adopt']

os.chdir("ml_adoption_timing")

### Cross-validation on policies

unique_policies = karch_2016['compnum'].unique()
...
for fold_idx, (train_idx, test_idx) in enumerate(kfold.split(unique_policies)):
    ...
    train_mask = karch_2016['compnum'].isin(train_policies)
    test_mask = karch_2016['compnum'].isin(test_policies)
    ...
    for policy in test_policies:
        ...
        policy_data = karch_2016[karch_2016['compnum'] == policy]
...
results_df.to_csv("figures/karch2016/karch_cdf_results_reglogit.csv", index = False)
...
with open('figures/karch2016/karch_maef1_reglogit.txt', 'w') as f:
